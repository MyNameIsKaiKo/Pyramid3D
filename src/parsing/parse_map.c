...
int	isopen_map(char **tab, size_t y, size_t x)
{
	...
	if (is_valid_c(top_case) || is_valid_c(left_case)
		|| is_valid_c(right_case) || is_valid_c(bottom_case))
...
int	check_line_map(char *line_read, t_map *map)
{
	...
	if (ft_strncmp(line_read, "\n", ft_strlen(line_read)) == 0 && map->y == 0)
		return (0);
	if (map->y > 0 && ft_strlen(line_read) > 1)
	{
		if (ft_strncmp(line_read, "\n", ft_strlen(line_read)) != 0
			&& ft_strlen(map->parse_map[map->y - 1]) == 0)
			return (print_error("A border line is incorrect"));
	}
